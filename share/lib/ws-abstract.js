'use strict'
  ; (function () {
    ...
    const resolveActSocketURI = function () {
      let o = /[?&]room=([^&\/]+)/.exec(location.search)
      return o && o[1]
    }
    ...
    class ActSocketLayer extends Layer {
      constructor() {
        super()
        this.type = 'act-socket'
        this.connected = false
        this.features = []

        this.status.locked = false
        this.room = resolveActSocketURI()
      }
      ...
      connect() {
        ...
        this.socketio = io.connect('', { query: 'room=' + this.room })
        this.socketio.on('update', e => {
          this.emit('data', JSON.parse(e))
        })
    ...
    window.ActSocketLayer = ActSocketLayer

    if (resolveSockURI()) {
      window.layer = new WSLayer()
    } else if (resolveActSocketURI()) {
      window.layer = new ActSocketLayer()
    } else {
